...
//                    则 P(i,j)=max[G(i),G(j)]或 P(i,j)=G(i)+G(j)
                    int R1=Math.abs((r1+2*r2+r3)-(r4+2*r5+r6));
                    int G1=Math.abs((g1+2*g2+g3)-(g4+2*g5+g6));
                    int B1=Math.abs((b1+2*b2+b3)-(b4+2*b5+b6));

                    int R2=Math.abs((r3+2*r8+r6)-(r1+2*r7+r4));
                    int G2=Math.abs((g3+2*g8+g6)-(g1+2*g7+g4));
                    int B2=Math.abs((b3+2*b8+b6)-(b1+2*b7+b4));
...
//                    int b=B1+B2;
        ...
        File sobel_result=new File(save_path);
        try {
            ImageIO.write(newImage,"jpg",sobel_result);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
